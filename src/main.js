...
while(shop = true) {
	...
	for (let i = 0; i < 10; i++) {
		indexNum = i + 1;
		console.log(indexNum + ". " + cars[i].brand + " [" + "$" + cars[i].bid + "]");
	}
	...
	console.log("Your car is " + cars[userInput - 1].brand + ". " + "Current price is " + "$" + cars[userInput-1].bid);
	console.log("=============================================")
	...
	while (bidLoop = true) {
		...
		if (isNaN(bidInput) || bidInput < 100) {
			...
			while (qbidprompt = true) {
				...
				if (qbid === 'y' || qbid === 'Y' || qbid === "n" || qbid === "N" || qbid === "") {
					break;
				} else {
	...
	while (shopping = true) {
		...
		if (buymore === "y" || buymore === "Y" || buymore === "" || buymore === "n" || buymore === "N") {
			break;
		} else {
			console.log("Invalid Input!")
